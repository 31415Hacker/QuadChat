...
- `dmSound.js` reads message-sound volume from `quadchat-message-sound-volume` (0–1, default `0.2`) at each play; the ringtone volume lives in `callRingtone.js` under `quadchat-ringtone-volume` (0–1, default `0.2`), applied on ringtone rebuild and on `startCallRingtone`.
...
- Group calls are now **creatable/joinable rooms**, tracked in an RTDB directory node `call-directory/<callKey>` = `{ callKey, title (≤50 chars), ownerId, ownerName, createdAt, passwordSalt?, passwordHash? }`. No `passwordHash` means public; a 64-hex SHA-256 of `salt:password` means private. Rules (`database.rules.json`): directory is read-only for signed-in users, writable to create (`!data.exists()`) or write/delete only by the owner (`data.child('ownerId').val() == auth.uid || newData.child('ownerId').val() == auth.uid` — RTDB rules have **no** `request.resource` object or `request` variable at all; that's Firestore-only, and using it makes the whole rule invalid so even deletes fail), shape validated via the `$callKey` `.validate` (note: RTDB string length is the `.length` *property*, not `.length()`).
- `deleteGroupCall(callKey)` removes the directory entry; if the caller is currently in that call it leaves first, then best-effort removes the orphaned `group-calls/<callKey>` signaling subtree (participants + mutes). It first tries a direct owner `remove()`; on `permission_denied` it falls back to `api/delete-call.js`, which serves as the admin path — the server verifies the caller is the directory owner (`call-directory/<callKey>.ownerId`) OR an admin/dev (`users/<caller>.isAdmin === true || isDeveloper === true`) and removes both nodes with the admin SDK (RTDB rules can't check Firestore, so admins need this server route). Directory removal errors are NOT swallowed — they propagate to `App.jsx` `handleDeleteCall`, which toasts a notification. Never re-add a `.catch(() => {})` around the directory `remove()` or a denied/blocked delete will silently do nothing.
