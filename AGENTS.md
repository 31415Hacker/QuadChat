...
- `deleteGroupCall(callKey)` removes the directory entry; if the caller is currently in that call it leaves first, then best-effort removes the orphaned `group-calls/<callKey>` signaling subtree (participants + mutes). Directory removal errors are NOT swallowed — they propagate to `App.jsx` `handleDeleteCall`, which toasts a notification. Never re-add a `.catch(() => {})` around the directory `remove()` or a denied/blocked delete will silently do nothing.
